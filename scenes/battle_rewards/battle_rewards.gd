...
const GOLD_ICON := preload("res://art/coinIcon.png")
const GOLD_TEXT := "%s Ouro"
const CARD_ICON := preload("res://art/add_card_icon.png")
const CARD_TEXT := "Adicionar nova pagina"
...
func _show_card_rewards() -> void:
	...
	var avaiable_cards: Array[Card] = character_stats.draftable_cards.cards.duplicate(true)
	for i in run_stats.card_rewards:
		...
		for rarity: Card.Rarity in card_rarity_weights:
			if card_rarity_weights[rarity] > roll:
				...
				var picked_card := _get_random_avaiable_card(avaiable_cards, rarity)
				card_reward_array.append(picked_card)
				avaiable_cards.erase(picked_card)
				break
...
func _get_random_avaiable_card(avaiable_cards: Array[Card], with_rarity: Card.Rarity) -> Card:
	var all_possible_cards := avaiable_cards.filter(
		func(card: Card):
